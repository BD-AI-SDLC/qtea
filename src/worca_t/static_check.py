...
Runs the SUT stack's own type-checker / linter against worca-generated test
...
Scope: violations reported on files OUTSIDE the worca-touched set are counted
but never fed to the violation-fixer. pyright/tsc will naturally type-check
transitive imports; worca-t will not autopatch user-owned app source.
...
from worca_t.stack_profile import StackProfile, detect_stack_profile, wrap_command
from worca_t.test_indexer import Violation
from worca_t.test_runner import execute_command, install_command_for
...
# depending on whether tsconfig.json is present and whether the worca-generated
# tests are .ts or .js.
#
# Coverage: all Python and all JS/TS stacks worca-t emits into. Java, Robot,
...
# `--skipLibCheck` mutes noise from third-party type definitions worca-t
...
# File extensions tsc can type-check. Used to filter the worca-touched set
...
# checker output rather than a worca-t-specific pattern violation.
...
def _filter_to_scope(
    ...
    """
    touched_resolved = {p.resolve() for p in worca_touched if p.exists()}
...
def _compose_argv(
    ...
    """Build the bare command string for ``tool`` against the worca-touched set.
    ...
    (e.g. tsc invoked but worca only touched .py files in a polyglot repo).
    """
    if tool == "pyright":
        py_files = sorted(
            p for p in worca_touched
            if p.exists() and p.suffix in _PY_EXTENSIONS
        )
        if not py_files:
            return "", "no Python files in worca-touched set"
        rel = [
    ...
    if tool == "tsc":
        ...
        if not js_ts_files:
            return "", "no JS/TS files in worca-touched set"
        rel = [
            str(p.relative_to(sut_root)).replace("\\", "/")
            for p in js_ts_files
            if p.is_relative_to(sut_root)
        ]
        # When tsconfig.json exists and the worca-touched set is ALL .ts (no
        ...
        # back down to worca files anyway, and TS projects with strict
...
def run_static_check(
    ...
    *,
    ...
    argv, skip_reason = _compose_argv(tool, sut_root, worca_touched)
    ...
    in_scope, out_of_scope = _filter_to_scope(raw, worca_touched, sut_root)
    return StaticCheckResult(
