...
worca-t creates a per-run isolation branch (``worca-t/run-<run_id>``) and
...
upstream — nothing worca-t writes ever lands on ``main`` directly.
...
This module is the single home for the git plumbing both ends rely on:
  ...
  - :func:`ensure_git_repo_and_branch` runs once, immediately after
    ...
    force-creates the worca-t branch. Idempotent — safe on resume.
...
_AUTHOR_NAME = "worca-t"
_AUTHOR_EMAIL = "worca-t@local"


def branch_name(run_id: str) -> str:
    """Canonical worca-t branch name for a run."""
    return f"worca-t/run-{run_id}"


def _git(sut_root: Path, *args: str, check: bool = True) -> subprocess.CompletedProcess:
    """Run ``git -c user.name=worca-t -c user.email=worca-t@local <args>`` in the SUT."""
...
def ensure_git_repo_and_branch(sut_root: Path, run_id: str) -> str:
    """Guarantee that ``sut_root`` is a git repo on branch ``worca-t/run-<run_id>``.
    ...
    - When it doesn't (local-path copy via ``shutil.copytree(...,
      ...
      so the worca-t branch has a parent commit to branch from.
    ...
    if not git_dir.exists():
        ...
        _git(sut_root, "commit", "--allow-empty", "-q", "-m", "worca-t baseline")
...
def commit_step(
    ...
    try:
        ...
        if not status.stdout.strip():
            ...
            return None
        subject = f"worca-t/step-{step_num:02d}: {step_name}"
        if message_detail:
