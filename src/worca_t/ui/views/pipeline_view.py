...
from worca_t.ui.components.log_viewer import build_log_viewer
from worca_t.ui.components.metrics_panel import build_metrics_panel
from worca_t.ui.components.phase_group import build_phase_group
from worca_t.ui.components.progress_header import build_progress_header
from worca_t.ui.state import AppState
from worca_t.ui.theme import BACKGROUND, CARD_BG, DIVIDER
...
def build_pipeline_view(page: ft.Page, state: AppState) -> ft.Container:
    ...
    def _on_step_click(step):
        from worca_t.ui.components.step_details_dialog import show_step_details_dialog
    ...
    from worca_t.ui.components.progress_header import fmt_elapsed
    ...
    def on_state_change() -> None:
        ...
        if hasattr(log_viewer, "content") and hasattr(log_viewer.content, "controls"):
            ...
            if len(cols) >= 2:
                ...
                if hasattr(log_list, "controls"):
                    from worca_t.ui.components.log_viewer import (
                        MAX_DISPLAY_LINES,
                        _build_log_line,
                    )
        ...
        if state.pending_hitl is not None:
            if shown["hitl"] is not state.pending_hitl:
                from worca_t.ui.components.hitl_dialog import show_hitl_dialog
        ...
        if state.pending_review_gate is not None:
            if shown["review"] is not state.pending_review_gate:
                from worca_t.ui.components.hitl_dialog import show_review_gate_dialog
