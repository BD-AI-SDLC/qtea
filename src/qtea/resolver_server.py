...
:func:`qtea.proxy.safe_subprocess_env`, which deliberately strips
...
``qtea resolve`` — silently failed at LLM time because the resolver
...
available. Pytest gets only two harmless env vars: ``QTEA_RESOLVER_PORT``
and ``QTEA_RESOLVER_TOKEN``. The pytest plugin connects to the loopback
...
log = logging.getLogger("qtea.resolver_server")
...
def _load_dev_pool_for_llm(path: Path | None) -> list[dict]:
    ...
    Falls back to ``QTEA_DEV_LOCATORS`` env var when ``path`` is None.
...
class ResolverServer:
    ...
    step that needed it::
        ...
        with ResolverServer(cache_dir=cache_dir, run_id=run_id) as srv:
            env["QTEA_RESOLVER_PORT"] = str(srv.port)
            env["QTEA_RESOLVER_TOKEN"] = srv.token
    ...
    def start(self) -> None:
        ...
        self._accept_thread = threading.Thread(
            target=self._accept_loop, name="qtea-resolver-accept", daemon=True,
        )
        self._accept_thread.start()
        log.info(
            "qtea.resolver_server_started host=%s port=%d cache_dir=%s dev_pool_size=%d",
    ...
    def stop(self, *, timeout: float = 5.0) -> None:
        ...
        log.info(
            "qtea.resolver_server_stopped requests=%d auth_failures=%d errors=%d",
    ...
    def _accept_loop(self) -> None:
        ...
        while not self._shutdown.is_set():
            ...
            if not self._conn_semaphore.acquire(blocking=False):
                ...
                log.warning("qtea.resolver_server_overload — connection dropped")
                with contextlib.suppress(OSError):
                    conn.close()
                continue
            t = threading.Thread(
                target=self._handle_connection, args=(conn,),
                name="qtea-resolver-conn", daemon=True,
            )
            t.start()
    ...
    def _handle_connection(self, conn: socket.socket) -> None:
        try:
            ...
            log.warning("qtea.resolver_server_conn_error %s: %s", type(e).__name__, e)
        finally:
            with contextlib.suppress(OSError):
    ...
    def _handle_request(self, conn: socket.socket) -> None:
        ...
        if not hmac.compare_digest(supplied, self.token):
            ...
            log.warning("qtea.resolver_server_auth_fail")
        ...
        try:
            ...
            with self._stats_lock:
                self._error_count += 1
            log.warning(
                "qtea.resolver_server_dispatch_error %s: %s",
                type(e).__name__, e,
            )
    ...
    def _send(self, conn: socket.socket, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            log.debug("qtea.resolver_server_send_failed %s", e)
...
def call_resolver(
    *,
    ...
    try:
        ...
        log.warning("qtea.resolver_client_transport_error %s", e)
        return None
    if not body:
        return None
    try:
        ...
        log.warning("qtea.resolver_client_bad_json %s", e)
        return None
    if not payload.get("ok"):
        log.warning(
            "qtea.resolver_client_server_error %s",
            payload.get("error"),
        )
        return None
    return payload
