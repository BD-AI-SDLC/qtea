...
def configure_logging(
    *,
    ...
    # Skipped when QTEA_UI_MODE is set — the desktop UI owns the screen,
    # so duplicating events into stderr is noise. (The JSONL audit trail is
    # still written below.)
    import os as _os
    if not _os.environ.get("QTEA_UI_MODE"):
    ...
    for h in list(root.handlers):
        ...
        # (e.g. the desktop-UI event bridge tagged with _qtea_keep).
        if getattr(h, "_qtea_keep", False):
            continue
...
def get_logger(name: str = "qtea") -> structlog.stdlib.BoundLogger:
    if not _CONFIGURED:
        configure_logging()
