...
# headroom for multi-failure heals. Overridable via QTEA_HEAL_MAX_TURNS.
HEAL_AGENT_MAX_TURNS: int = int(os.environ.get("QTEA_HEAL_MAX_TURNS", "40"))
...
# rewrite). Overridable via QTEA_AUTOFIX_MAX_TURNS.
AUTOFIX_MAX_TURNS: int = int(os.environ.get("QTEA_AUTOFIX_MAX_TURNS", "40"))
...
def get_settings() -> Settings:
    ...
    return Settings(
        claude_bin=os.environ.get("QTEA_CLAUDE_BIN", "claude"),
...
def use_vertex_backend() -> bool:
    """Return True when qtea should route LLM calls through Vertex AI.
...
def package_resource_root() -> Path:
    ...
    Resolution order:
      1. ``QTEA_RESOURCE_ROOT`` env var (developer override — point at the
    ...
    """
    override = os.environ.get("QTEA_RESOURCE_ROOT")
    if override:
        p = Path(override)
        if p.exists():
            return p
    pkg_resources = Path(__file__).parent / "_resources"
    if pkg_resources.exists() and (pkg_resources / "schemas").exists():
