...
# TS/JS: `const NAME = tbd("...")`, `const NAME = qtea.tbd("...")`,
# or bare `tbd("...")` / `qtea.tbd("...")`. Also matches template literals
...
def _iter_source_files(p: Path) -> Iterable[Path]:
    ...
    # a qtea workspace at ``~/.qtea/<run>/sut/...`` trips the
    # hidden-dir filter on every file because ``.qtea`` is one of the
