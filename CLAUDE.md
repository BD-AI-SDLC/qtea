...
Action-time `TimeoutError` → bundle-fallback walk → cache invalidate (or **dev-pool quarantine**, see below) → re-resolve once → replay → fall through to `polyglot-test-fixer` heal agent. `WORCA_T_NO_LLM_RESOLVE=1` disables tiers 4-5 + the heal agent symmetrically (CI default for zero-LLM-spend). Async Playwright is fully patched alongside sync. Full env-var list + implementation: the runtime template docstring.
